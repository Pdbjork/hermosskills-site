...
| Auth errors | Run `wrangler login` — or, on a headless host without a browser, set `CLOUDFLARE_API_TOKEN` (see Headless / CI auth below) |
| `Authentication error [code: 10000]` and `account_id` mismatch warning | The `account_id` in `wrangler.toml` doesn't match the authenticated account. Run `wrangler whoami` to get the real ID, replace it, or just delete the line — wrangler will use the only authenticated account. |
...
### Headless / CI auth — the `CLOUDFLARE_API_TOKEN` pattern

`wrangler login` opens a browser tab for an OAuth dance. **That fails on a headless host** (server with no DISPLAY, SSH session, Codespace, CI runner, container). Don't waste time trying to make it work — there is no fallback inside `wrangler login` itself.

Instead, create a **Custom API Token** in the Cloudflare dashboard with the scopes your task needs, then load it into the shell as `CLOUDFLARE_API_TOKEN`. From there `wrangler deploy` (and `wrangler kv namespace create`, `wrangler secret put`, etc.) work non-interactively.

**Pitfall:** do NOT pass the token as a CLI argument. It lands in shell history, process listings (`ps aux`), and any logging the shell does. Always `read -rs` it from stdin or set it as an env var for the duration of one shell session.

Minimal pattern:

```bash
# Token from https://dash.cloudflare.com/profile/api-tokens
# Required scopes depend on task; for a typical Worker deploy with KV + secrets:
#   - Account.Workers Scripts: Edit
#   - Account.Workers KV Storage: Edit
#   - Account.Account Settings: Read
read -rs CLOUDFLARE_API_TOKEN   # paste, Enter; nothing echoes
export CLOUDFLARE_API_TOKEN

npx wrangler whoami              # verify: should show your account + scopes
npx wrangler deploy

unset CLOUDFLARE_API_TOKEN       # before exiting the shell
```

For CI, the same env var pattern works in the runner's secret store.

**`account_id` in `wrangler.toml` is a footgun.** If a stale or wrong `account_id` is in your config, wrangler authenticates the token fine but then errors with `Authentication error [code: 10000]` and prints a warning: *"The `account_id` in your Wrangler configuration (...) does not match any of your authenticated accounts."* The fix is to either:
1. Update the `account_id` to match `wrangler whoami`'s output, **or**
2. Just delete the `account_id` line entirely — wrangler uses the only authenticated account.

Prefer (2) unless you genuinely have multi-account setups that need disambiguation.

### Interactive `wrangler secret put` on a headless host

`wrangler secret put NAME` opens an interactive prompt for the secret value. On a headless host, pipe the value into stdin:

```bash
# Read from a local file (best for tokens that are already in a .env)
grep -E '^STRIPE_SECRET_KEY=*** .env | cut -d= -f2- | npx wrangler secret put STRIPE_SECRET_KEY

# Or type/paste once
read -rs VALUE
echo "$VALUE" | npx wrangler secret put SOME_SECRET
unset VALUE
```

Never `echo` the raw value into a command that lands in shell history or process listing. The pipe-from-stdin pattern is safe because the value is not an argument.
